function navigationStarPlot(mapFile, goal, k, lambda, locs, start, dim)
% inputs:
...
% outputs:
%   plot of navigation function and contours for star shapes
%
%   Cornell University
...
%   Scher, Guy
    ...
    figure('Name','NavigationStarPlot'); subplot(211);
%     j=1;
...
%         subplot(230+j);
    ...
    for i = 1:size(locs,1)
        Nav = starPoint(map, goal, k, lambda, locs(i,:)');
        N(i) = Nav;
    end
    if(~isempty(start))
...
%     subplot(230+j);
    ...
    title(['Star Navigation function - Contours & Gradients k=' num2str(k)]); xlabel('X [m]'); ylabel('Y [m]'); grid on;
